...
function DataUploadPanel({
  ...
  const loadTkPeriods = useCallback(async () => {
    try {
      const r = await api.getTiktokExportList(storeCode, "board");
  ...
  async function uploadTiktok() {
    ...
    setStatus("loading"); setMsg("");
    try {
      const r = await api.importTiktokExport(tiktokFile, storeCode, "board");
    ...
    <div className={`${t.card} rounded-xl overflow-hidden`}>
      ...
      <div className="p-5">
        {activeTab === "analytics" ? (
          ...
          <>
            <p className={`text-sm ${t.t3} mb-4`}>
              Upload a TikTok product analytics export file (<strong>product_list_All_*.xlsx</strong>). Data uploaded here is only used for this board's product cards.
            </p>
            <input type="file" accept=".xlsx,.xls"
            ...
            {tkPeriods.length > 0 ? (
              <div className="mt-5">
                <h4 className={`text-xs font-semibold ${t.t4} uppercase mb-2`}>Imported Periods (Board)</h4>
                <div className="space-y-1.5">
                  {tkPeriods.map((p, i) => (
                    <div key={i} className="flex items-center gap-3 text-xs bg-sky-50 rounded-lg px-3 py-2 hover:bg-sky-100 transition-colors">
                      ...
                      <button
                        onClick={async () => {
                          if (confirm(`Delete board analytics for ${p.period_start} → ${p.period_end}?`)) {
                            try { await api.deleteTiktokExport(storeCode, p.period_start, p.period_end, "board"); loadTkPeriods(); }
              ...
              <p className={`text-xs ${t.t4} mt-4`}>No board analytics uploaded yet for this store.</p>
            )}
          </>
        ) : (
          <>
